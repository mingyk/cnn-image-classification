...
tensor im2col(tensor im, size_t size_y, size_t size_x, size_t stride, size_t pad)
{
    ...
    for(i = 0; i < rows; ++i){
        // convert 1D to 3D
        int x = i % size_x - pad;
        int y = (i / size_x) % size_y - pad;
        int gc = i / (size_x*size_y);
        // printf("L(%d,%d)\n", x, y);
        for(j = 0; j < im_h + 2*pad - size_y + 1; j+=stride){ // vertical
            for(k = 0; k < im_w + 2*pad - size_x + 1; k+=stride){ // horizontal
                int index = j / stride * res_w + k / stride;
                int gx = x + k;
                int gy = y + j;
                // printf(" G(%d,%d)\n", gx, gy);
                float val;
                if(gy >= im_h || gx >= im_w || gy < 0 || gx < 0){
                    val = 0;
                }else{
                    val = im.data[gx + im_w*(gy + im_h*gc)];
                }
                int idx = i*cols + index;
                col.data[idx] = val;
                // printf("%d: %d\n", idx, val);
            }
        }
    ...
    for(i = 0; i < rows; ++i){
        ...
        for(j = 0; j < im_h + 2*pad - size_y + 1; j+=stride){
            for(k = 0; k < im_w + 2*pad - size_x + 1; k+=stride){
                ...
                int gx = x + k;
                int gy = y + j;
                float old_val = im.data[gx + im_w*(gy + im_h*gc)];
                float new_val = col.data[i*cols + index];
                float val = old_val + new_val;
                int idx = gx + im_w*(gy + im_h*gc);
                if(gx >= 0 && gy >= 0 && gx < im_w && gy < im_h){
                    im.data[idx] = val;
                }
            }
        }
