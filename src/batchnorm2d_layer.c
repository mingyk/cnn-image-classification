...
tensor normalize2d(tensor x, tensor m, tensor v)
{
    ...
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    int index = i + w*(j + h*(k + b*c));
                    y.data[index] = (x.data[index] - m.data[k])/sqrt(v.data[k] + 0.00001f);
                }
            }
        }
    }
...
tensor delta_mean2d(tensor dy, tensor v)
{
    ...
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    dm.data[k] += dy.data[i + w*(j + h*(k + b*c))] * (-1 / sqrt(v.data[k] + 0.00001f));
                }
            }
        }
    }
...
int i, j, k, b;
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    dv.data[k] += dy.data[i + w*(j + h*(k + b*c))] * (x.data[i + w*(j + h*(k + b*c))] - m.data[k]) * ((-1.0/2.0) * pow(v.data[k] + 0.00001f, (-3.0/2.0)));
                }
            }
        }
    }
...
tensor delta_batchnorm2d(tensor dy, tensor dm, tensor dv, tensor m, tensor v, tensor x)
{
    ...
    for(b = 0; b < n; ++b){
        for(k = 0; k < c; ++k){
            for(j = 0; j < h; ++j){
                for(i = 0; i < w; ++i){
                    float dldy = dy.data[i + w*(j + h*(k + b*c))] * (1.0 / sqrt(v.data[k] + 0.00001f));
                    float dldv = dv.data[k] * ((2.0/num)*(x.data[i + w*(j + h*(k + b*c))] - m.data[k]));
                    float dldm = dm.data[k] * (1.0/num);
                    int index = i + w*(j + h*(k + b*c));
                    dx.data[index] = dldy + dldv + dldm;
                }
            }
        }
    }
